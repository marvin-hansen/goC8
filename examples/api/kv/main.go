...
func main() {
	...
	utils2.CheckErrorLog(err, "Error creatin KV collection"+collectionName)
	utils2.PrintRes(res, verbose)

	println("Get all KV collection: ")
	res, err = c.KV.GetAllKVCollections(fabric)
	utils2.CheckErrorLog(err, "Error getting KV collection"+collectionName)
	utils2.PrintRes(res, verbose)
	...
	utils2.CheckErrorLog(errSet, "Error setting values into KV collection"+collectionName)
	utils2.PrintRes(resSet, verbose)

	println("Count all items in KV collection: ")
	resCount, errCount := c.KV.CountKVCollection(fabric, collectionName)
	utils2.CheckErrorLog(errCount, "Error counting values into KV collection"+collectionName)
	utils2.PrintRes(resCount, verbose)
	...
	utils2.CheckErrorLog(errGetKeys, "Error getting all keys from KV collection"+collectionName)
	utils2.PrintRes(resGetKeys, verbose)
	...
	utils2.CheckErrorLog(errGetValues, "Error getting all values from KV collection"+collectionName)
	utils2.PrintRes(resGetValues, verbose)

	println("Get single value from KV collection: ")
	key := "value1"
	resOneVal, errOneVal := c.KV.GetValue(fabric, collectionName, key)
	utils2.CheckErrorLog(errOneVal, "Error getting one value from KV collection"+collectionName)
	utils2.PrintRes(resOneVal, verbose)

	println("Delete single value from KV collection: ")
	resOneDel, errOneDel := c.KV.DeleteValue(fabric, collectionName, key)
	utils2.CheckErrorLog(errOneDel, "Error deleting one value from KV collection"+collectionName)
	utils2.PrintRes(resOneDel, verbose)
	...
	utils2.CheckErrorLog(errUpd, "Error updating values into KV collection"+collectionName)
	utils2.PrintRes(resUpd, verbose)
	...
	if delete {
		...
		utils2.CheckErrorLog(errTruncate, "Error truncating all values in KV collection"+collectionName)

		_, err = c.KV.DeleteKVCollection(fabric, collectionName)
		utils2.CheckErrorLog(errTruncate, "Error deleting KV collection"+collectionName)
	}
